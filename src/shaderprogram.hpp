...
struct ShaderProgramCreateInfo {
  vk::Device device;
  std::span<std::uint32_t const> vert_spirv;
  std::span<std::uint32_t const> frag_spirv;
...
class ShaderProgram {
  ...
  private:
    static void set_viewport_scissor(vk::CommandBuffer command_buffer, glm::ivec2 framebuffer);
