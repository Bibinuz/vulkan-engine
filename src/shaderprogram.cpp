...
namespace lvk {
namespace {
constexpr auto to_vkbool(bool const value) { return value ? vk::True : vk::False; }
} // namespace
...
ShaderProgram::ShaderProgram(CreateInfo const &create_info)
    ...
    auto const create_shader_ci = [&create_info](std::span<std::uint32_t const> spriv) {
        auto ret = vk::ShaderCreateInfoEXT{};
        ret.setCodeSize(spriv.size_bytes())
            .setPCode(spriv.data())
    ...
    auto shader_cis = std::array{
        create_shader_ci(create_info.vert_spirv),
        create_shader_ci(create_info.frag_spirv),
    };
    shader_cis.at(0)
        .setStage(vk::ShaderStageFlagBits::eVertex)
        .setNextStage(vk::ShaderStageFlagBits::eFragment);
    shader_cis.at(1).setStage(vk::ShaderStageFlagBits::eFragment);
