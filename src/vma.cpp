...
auto vma::create_image(const ImageCreateInfo &create_info, 
...
auto vma::create_sampled_image(ImageCreateInfo const& create_info,
  ...
  auto const stagin_buffer = create_buffer(buffer_ci, BufferMemoryType::Host, bitmap.bytes.size_bytes());
  if (!ret.get().image || !stagin_buffer.get().buffer) { return {}; }
  ...
  // Diferent a la guia, crec que la versió correcta es aquesta
  subresource_layers.setAspectMask(vk::ImageAspectFlagBits::eColor)
    .setLayerCount(1)
    .setMipLevel(mip_levels);
