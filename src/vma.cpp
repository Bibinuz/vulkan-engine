...
void Deleter::operator()(VmaAllocator allocator) const noexcept { vmaDestroyAllocator(allocator); }
...
auto vma::create_allocator(vk::Instance const instance, vk::PhysicalDevice const physical_device,
...
auto vma::create_buffer(BufferCreateInfo const &create_info, BufferMemoryType const memory_type,
    ...
    buffer_ci.setQueueFamilyIndices(create_info.queue_family).setSize(size).setUsage(usage);
    auto vma_buffer_ci = static_cast<VkBufferCreateInfo>(buffer_ci);

    VmaAllocation allocation{};
    VkBuffer buffer{};
    auto allocation_info = VmaAllocationInfo{};
    auto const result = vmaCreateBuffer(create_info.allocator, &vma_buffer_ci, &allocation_ci,
...
auto vma::create_device_buffer(BufferCreateInfo const &create_info, CommandBlock command_block,
                               ByteSpans const &byte_spans) -> Buffer {
    auto const total_size = std::accumulate(
        byte_spans.begin(), byte_spans.end(), 0uz,
        [](std::size_t const n, std::span<std::byte const> bytes) { return n + bytes.size(); });
...
auto vma::create_image(const ImageCreateInfo &create_info, vk::ImageUsageFlags usage,
                       std::uint32_t levels, vk::Format format, vk::Extent2D extent) -> Image {
    ...
    VkImage image{};
    VmaAllocation allocation{};
    auto const result = vmaCreateImage(create_info.allocator, &vk_image_ci, &allocation_ci, &image,
                                       &allocation, {});
    if (result != VK_SUCCESS) {
...
auto vma::create_sampled_image(ImageCreateInfo const &create_info, CommandBlock command_block,
                               Bitmap const &bitmap) -> Image {
    // Create image
    auto const mip_levels = 1;
    ...
    auto const stagin_buffer =
        create_buffer(buffer_ci, BufferMemoryType::Host, bitmap.bytes.size_bytes());
    if (!ret.get().image || !stagin_buffer.get().buffer) {
        return {};
    }
    std::memcpy(stagin_buffer.get().mapped, bitmap.bytes.data(), bitmap.bytes.size_bytes());
    ...
    copy_info.setDstImage(ret.get().image)
        ...
        .setSrcBuffer(stagin_buffer.get().buffer)
